        ...
        implements JdbcConnectionFactory
{
    ...
    public Connection getConnection(final JdbcCredentialProvider jdbcCredentialProvider)
    {
        try {
            ...
            if (jdbcCredentialProvider instanceof RdsSecretsCredentialProvider) {
                Matcher secretMatcher = SECRET_NAME_PATTERN.matcher(databaseConnectionConfig.getJdbcConnectionString());
                var credentials = jdbcCredentialProvider.getCredential();
                final String secretReplacement = String.format("user=%s&password=%s", credentials.getUser(), 
            ...
            } else if (jdbcCredentialProvider != null) {
                var credentials = jdbcCredentialProvider.getCredential();
                var connectionString = databaseConnectionConfig.getJdbcConnectionString().replace("user=%s", String.format("user=%s", credentials.getUser()));
                derivedJdbcString = connectionString.replace("password=%s", String.format("password=%s", encodeValue(credentials.getPassword())));
            }
            else {
