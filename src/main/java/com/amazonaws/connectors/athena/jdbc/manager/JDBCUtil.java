...
public final class JDBCUtil
{
     ...
     */
    public static Pair<DatabaseConnectionConfig, DatabaseConnectionConfig> getSingleDatabaseConfigFromEnv(final JdbcConnectionFactory.DatabaseEngine databaseEngine)
    {
        List<Pair<DatabaseConnectionConfig, DatabaseConnectionConfig>> databaseConnectionConfigs = DatabaseConnectionConfigBuilder.buildFromSystemEnv();

        for (Pair<DatabaseConnectionConfig, DatabaseConnectionConfig> databaseConnectionConfig : databaseConnectionConfigs) {
            if (DatabaseConnectionConfigBuilder.DEFAULT_CONNECTION_STRING_PROPERTY.equals(databaseConnectionConfig.getLeft().getCatalog())
                    && databaseEngine.equals(databaseConnectionConfig.getLeft().getType())) {
    ...
    public static Map<String, JdbcMetadataHandler> createJdbcMetadataHandlerMap(final Map<String, String> properties)
    {
        ...
        var databaseConnectionConfigs = new DatabaseConnectionConfigBuilder().properties(properties).build();
        ...
        for (var databaseConnectionConfig : databaseConnectionConfigs) {
            var jdbcMetadataHandler = createJdbcMetadataHandler(databaseConnectionConfig.getRight());
            metadataHandlerMap.put(databaseConnectionConfig.getLeft().getCatalog(), jdbcMetadataHandler);
    ...
    public static Map<String, JdbcRecordHandler> createJdbcRecordHandlerMap(final Map<String, String> properties)
    {
        ...
        for (var databaseConnectionConfig : databaseConnectionConfigs) {
            JdbcRecordHandler jdbcRecordHandler = createJdbcRecordHandler(databaseConnectionConfig.getLeft());
            recordHandlerMap.put(databaseConnectionConfig.getLeft().getCatalog(), jdbcRecordHandler);
